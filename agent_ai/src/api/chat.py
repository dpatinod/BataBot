...
@chat_router.post("/message", response_model=ResponseHTTPChat)
async def endpoint_message(request: RequestHTTPChat):
    ...
    if pdf_chat_agent is None:
        raise HTTPException(status_code=500, detail="pdf_chat_agent no está inicializado")
...
@chat_router.post("/attachment", response_model=dict)  # Ajusta el response_model según tu modelo
...
@chat_router.post("/sessions", response_model=ResponseHTTPSessions)
...
@chat_router.post("/get_one_session", response_model=ResponseHTTPOneSession)
